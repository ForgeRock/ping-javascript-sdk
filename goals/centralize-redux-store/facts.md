...
- `oidc` lazily injects its own reducers (`oidcApi`) into the shared store at `oidc()` init time — the owning store (davinci/journey) does NOT pre-provision oidc reducers.
...
- All affected packages pass `pnpm lint`, `pnpm build`, and `pnpm test` after the changes.
