...
import { ActionTypes } from './request.effect.enums.js';
...
function getActionType(endpoint: string): ActionTypes {
  switch (endpoint) {
    case 'start':
      return ActionTypes.DaVinciStart;
    case 'next':
      return ActionTypes.DaVinciNext;
    case 'flow':
      return ActionTypes.DaVinciFlow;
    case 'success':
      return ActionTypes.DaVinciSuccess;
    case 'error':
      return ActionTypes.DaVinciError;
    default:
      return ActionTypes.DaVinciFailure;
  }
}
 ...
 * @returns
 */
export function initQuery(fetchArgs: FetchArgs, endpoint: string) {
  ...
  const queryApi = {
    applyMiddleware(middleware: RequestMiddleware[] | undefined) {
      ...
      const runMiddleware = middlewareWrapper(modifiedRequest, { type: getActionType(endpoint) });
