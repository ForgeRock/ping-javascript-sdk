...
export function handleResponse(
  ...
  if (cacheEntry.isError && cacheEntry.error.status >= 500) {
    logger.debug('Response of 5XX indicates unrecoverable failure');
  ...
  if (cacheEntry.isError && cacheEntry.error.status >= 400 && cacheEntry.error.status < 500) {
    ...
    if (data.code === 1999 || data.code === 'requestTimedOut') {
      logger.debug('Error is a client-side timeout');
      dispatch(nodeSlice.actions.failure({ data, requestId, httpStatus: cacheEntry.error.status }));
    ...
    if (
      ...
      logger.debug('Error is a PingOne Authentication Connector unrecoverable failure');
      dispatch(nodeSlice.actions.failure({ data, requestId, httpStatus: cacheEntry.error.status }));
  ...
  if (cacheEntry.isSuccess && 'error' in cacheEntry.data) {
    logger.debug('Response with `isSuccess` but `error` property indicates unrecoverable failure');
...
export function authorize(
  ...
  if (serverSlice && '_links' in serverSlice) {
    ...
    if (continueUrl) {
      ...
      logger.debug('No url found in collector, social login needs a url in the collector');
      return {
        error: {
          message:
