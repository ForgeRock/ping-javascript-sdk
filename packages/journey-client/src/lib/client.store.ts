...
export async function journey({
  ...
  // Convert the well-known response into resolved config (baseUrl + paths)
  // before dispatching, so the reducer remains a pure function.
  let resolvedServerConfig;
  try {
    resolvedServerConfig = convertWellknown(wellknownResponse);
  } catch (error) {
    const message =
      error instanceof Error ? error.message : 'Failed to process well-known response';
    log.error(message);
    throw new Error(message);
  }
